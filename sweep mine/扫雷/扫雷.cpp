...
bool isWin() {
  for (int i = 0; i < N; i++)
  {
	for (int j = 0; j < N; j++)
	{
	  if (map[i][j] != open && valueMap[i][j] != mine)
	  {
		return false;
	  }
	}
...
void start()
{
  ...
  while (true)
  {
	...
	if (isGameOver)  //游戏失败
	{
	  int a = MessageBox(GetForegroundWindow(), _T("踩到雷了:是否重新游戏"),
		_T("游戏失败"), MB_YESNO);
	  if (a == IDYES)  //表示按下了是
		init_map();
	  else if (a == IDNO)
	...
	if (isWin())
	{
	  int a = MessageBox(GetForegroundWindow(), _T("胜利:是否重新游戏"),
		_T("游戏胜利"), MB_YESNO);
	  if (a == IDYES)  //表示按下了是
		init_map();
	  else if (a == IDNO)
